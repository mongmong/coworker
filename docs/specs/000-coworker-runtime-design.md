...
**Conflict detection:** Before starting a plan, the scheduler enforces declared `blocks_on` dependencies. If the manifest includes optional path reservations (`paths.likely_touch`), the scheduler also compares those paths with other active plans and produces a warning or checkpoint according to policy. Without path reservations, Coworker detects conflicts later through git merge/rebase status and dirty-worktree checks; it does not pretend that `blocks_on` alone proves file independence.
...
- Works consistently for MCP-pull clients such as Claude Code and Codex
- Session state is respected (agent polls when ready)

OpenCode is different: its primary path is HTTP dispatch through `opencode serve`, with MCP pull retained only as a possible hybrid path if the Phase 0 spike shows it is useful.
...
| Spike | Goal | Effort |
|---|---|---|
| Claude Code persistent + MCP pull | Determine whether skill-driven polling and `orch.job.complete` round-trips are solid enough for persistent mode, and whether any wake mechanism is reliable enough to keep an idle session useful | ~1 day |
| Codex persistent + MCP pull | Same for Codex; verify its MCP client behavior and whether persistent pull is viable or should shrink to ephemeral-only | ~1 day |
...
- Three role integrations: Claude Code interactive + worker, OpenCode interactive + worker, Codex worker-only by default unless the Phase 0 spike proves interactive pull is reliable
...
- Supervisor contract with blocking enforcement; supervisor quality advisory-first with a small block-capable category allowlist
...
- Bulletin-board reactive dispatch beyond the core workflow engine; recommended as an optional V1.5 addon (see `docs/specs/003-bulletin-board-reactive-dispatch.md`)
...
2. **MCP notification support** — whether Claude Code / Codex support server-initiated notifications reliably. Determined by spike. If unsupported, the runtime must either prove a CLI-specific wake mechanism or downgrade that CLI to explicit-turn / ephemeral-only behavior rather than assuming tmux wake works.
